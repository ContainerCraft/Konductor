...
def deploy_talos_cluster(
    ...
    controlplane_config = get_talos_config(
        config_talos.get("controlplane", {}), "controlplane"
    )
...
def get_talos_config(config_talos_cluster: dict, node_type: str) -> dict:
    """
    ...
    common_talos_defaults = {
        "namespace": "default",
        "image": config_talos_cluster.get(
            "image", "docker.io/containercraft/talos:1.7.6"
        ),
    ...
    if node_type == "controlplane":
        ...
        if controlplane_config == "single":
            ...
            pulumi.log.error(
                f"Unrecognized controlplane replica config. Expected 'single' or 'ha', got: {controlplane_config}"
            )
        ...
        controlplane_defaults = {
            ...
            "memory_size": config_talos_cluster.get(
                "memory_size", "2"
            ),  # Memory in GiB
            "root_disk_size": config_talos_cluster.get(
                "root_disk_size", "32"
            ),  # Root disk size in GiB
            "empty_disk_size": config_talos_cluster.get(
                "empty_disk_size", "0"
            ),  # Empty disk size in GiB
...
def deploy_talos_cluster_controlplane(
    config_vm, k8s_provider: k8s.Provider, depends_on: pulumi.Output[list], parent
):
    """
...
def deploy_talos_cluster_workers(
    config_vm, k8s_provider: k8s.Provider, depends_on, parent
):
    """
...
def generate_talos_vmpool_spec(
    ...
    spec = {
        "replicas": replicas,
        "selector": {"matchLabels": {"kubevirt.io/vmpool": vm_pool_name}},
        "virtualMachineTemplate": {
            "metadata": {"labels": {"kubevirt.io/vmpool": vm_pool_name}},
            "spec": {
                "running": running,
                "template": {
                    "metadata": {"labels": {"kubevirt.io/vmpool": vm_pool_name}},
                    "spec": {
                        ...
                        "domain": {
                            "cpu": {"cores": cpu_cores},  # Use configured CPU cores
                            "resources": {
                                "requests": {
                            ...
                            "devices": {
                                ...
                                "interfaces": [{"name": "eth0", "bridge": {}}],
                            },
                        },
                        "volumes": [
                            {
                                "name": "talos-root-disk",
                                "dataVolume": {"name": f"{vm_pool_name}-root-dv"},
                            }
                        ],
                    },
                },
                "dataVolumeTemplates": [
                    {
                        "metadata": {"name": f"{vm_pool_name}-root-dv"},
                        "spec": {
                            "storage": {
    ...
    if int(empty_disk_size) > 0:
        spec["virtualMachineTemplate"]["spec"]["template"]["spec"]["domain"]["devices"][
            "disks"
        ].append({"name": "talos-empty-disk", "disk": {"bus": "virtio"}})

        spec["virtualMachineTemplate"]["spec"]["template"]["spec"]["volumes"].append(
            {
                "name": "talos-empty-disk",
                "dataVolume": {"name": f"{vm_pool_name}-empty-dv"},
            }
        )
        ...
        spec["virtualMachineTemplate"]["spec"]["dataVolumeTemplates"].append(
            {
                "metadata": {"name": f"{vm_pool_name}-empty-dv"},
                "spec": {
                    "storage": {
                        "accessModes": ["ReadWriteOnce"],
                        "resources": {
