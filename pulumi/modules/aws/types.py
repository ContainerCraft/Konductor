...
class IAMUserConfig(BaseModel):
    ...
    groups: List[str] = Field(default_factory=list, description="IAM groups the user belongs to.")
    policies: List[str] = Field(default_factory=list, description="IAM policy ARNs attached to the user.")
...
class ControlTowerConfig(BaseModel):
    ...
    def validate_control_tower_fields(cls, v, values):
        if v:
            ...
            if missing:
                raise ValueError(f"Missing fields for Control Tower: {', '.join(missing)}")
        return v
...
class TenantAccountConfig(BaseModel):
    ...
    aws: Dict[str, Any] = Field(default_factory=dict, description="AWS-specific configuration for the tenant account.")
    tags: Dict[str, str] = Field(default_factory=dict, description="Tags for resources in the tenant account.")
...
class AWSConfig(BaseModel):
    ...
    iam_users: List[IAMUserConfig] = Field(default_factory=list, description="IAM user configurations.")
    landingzones: List[TenantAccountConfig] = Field(default_factory=list, description="Tenant account configurations.")
    global_tags: GlobalTags = Field(default_factory=GlobalTags, description="Global tags for all resources.")
    compliance: ComplianceConfig = Field(default_factory=ComplianceConfig, description="Compliance configuration.")
    version: str = Field(default="0.0.1", description="Version of the local AWS module.")
