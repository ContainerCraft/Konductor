...
allowed_ssh_cidr = (
    config.get("allowed_ssh_cidr") or "203.0.113.0/24"
)  # Example IP range
...
eks_role = aws_native.iam.Role(
    f"eksRole-{stack_name}",
...
node_role = aws_native.iam.Role(
    f"nodeRole-{stack_name}",
    assume_role_policy_document=json.dumps(
        {
            "Version": "2012-10-17",
            "Statement": [
...
vpc = aws_native.ec2.VPC(
    ...
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"vpc-{stack_name}")],
)

# Create Internet Gateway
igw = aws_native.ec2.InternetGateway(
    f"igw-{stack_name}",
    vpc_id=vpc.id,
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"igw-{stack_name}")],
)

# Create Elastic IP for NAT Gateway
nat_eip = aws_native.ec2.EIP(
    f"natEip-{stack_name}",
    domain="vpc",
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"natEip-{stack_name}")],
)
...
for i, az in enumerate(azs):
    public_subnet = aws_native.ec2.Subnet(
        f"publicSubnet-{stack_name}-{i}",
        vpc_id=vpc.id,
        cidr_block=f"10.0.{i}.0/24",
        map_public_ip_on_launch=True,
        availability_zone=az,
        tags=[
            aws_native.ec2.TagArgs(key="Name", value=f"publicSubnet-{stack_name}-{i}")
        ],
    )
    public_subnets.append(public_subnet)

    private_subnet = aws_native.ec2.Subnet(
        f"privateSubnet-{stack_name}-{i}",
        vpc_id=vpc.id,
        cidr_block=f"10.0.{i+10}.0/24",
        map_public_ip_on_launch=False,
        availability_zone=az,
        tags=[
            aws_native.ec2.TagArgs(key="Name", value=f"privateSubnet-{stack_name}-{i}")
        ],
    )
    private_subnets.append(private_subnet)

# Create NAT Gateway
nat_gateway = aws_native.ec2.NatGateway(
    f"natGateway-{stack_name}",
    subnet_id=public_subnets[0].id,
    allocation_id=nat_eip.allocation_id,
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"natGateway-{stack_name}")],
)

# Create Route Tables
public_route_table = aws_native.ec2.RouteTable(
    f"publicRouteTable-{stack_name}",
    vpc_id=vpc.id,
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"publicRouteTable-{stack_name}")],
)

private_route_table = aws_native.ec2.RouteTable(
    f"privateRouteTable-{stack_name}",
    vpc_id=vpc.id,
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"privateRouteTable-{stack_name}")],
)

# Create Routes
public_route = aws_native.ec2.Route(
    f"publicRoute-{stack_name}",
    route_table_id=public_route_table.id,
    destination_cidr_block="0.0.0.0/0",
    gateway_id=igw.id,
)

private_route = aws_native.ec2.Route(
    f"privateRoute-{stack_name}",
...
for i, subnet in enumerate(public_subnets):
    aws_native.ec2.SubnetRouteTableAssociation(
        f"publicSubnetRouteTableAssociation-{stack_name}-{i}",
        subnet_id=subnet.id,
        route_table_id=public_route_table.id,
    )
...
for i, subnet in enumerate(private_subnets):
    aws_native.ec2.SubnetRouteTableAssociation(
        f"privateSubnetRouteTableAssociation-{stack_name}-{i}",
        subnet_id=subnet.id,
        route_table_id=private_route_table.id,
    )
...
cluster_security_group = aws_native.ec2.SecurityGroup(
    f"clusterSecurityGroup-{stack_name}",
    vpc_id=vpc.id,
    group_description="EKS cluster security group",
    tags=[
        aws_native.ec2.TagArgs(key="Name", value=f"clusterSecurityGroup-{stack_name}")
    ],
)

node_security_group = aws_native.ec2.SecurityGroup(
    f"nodeSecurityGroup-{stack_name}",
    vpc_id=vpc.id,
    group_description="EKS node group security group",
    tags=[aws_native.ec2.TagArgs(key="Name", value=f"nodeSecurityGroup-{stack_name}")],
)

# Security Group Rules
aws_native.ec2.SecurityGroupIngress(
    f"nodeToClusterAPIServer-{stack_name}",
...
aws_native.ec2.SecurityGroupEgress(
    f"clusterToNodeGroup-{stack_name}",
    group_id=cluster_security_group.id,
    ip_protocol="tcp",
    from_port=1025,
    to_port=65535,
...
aws_native.ec2.SecurityGroupIngress(
    f"clusterToNodeGroupIngress-{stack_name}",
...
aws_native.ec2.SecurityGroupIngress(
    f"nodeToNodeIngress-{stack_name}",
    group_id=node_security_group.id,
    ip_protocol="-1",
    from_port=0,
    to_port=0,
...
eks_cluster = aws_native.eks.Cluster(
    f"eksCluster-{stack_name}",
    name=eks_cluster_name,
    role_arn=eks_role.arn,
    version="1.26",
    encryption_config=[
        aws_native.eks.ClusterEncryptionConfigArgs(
            provider=aws_native.eks.ProviderArgs(key_arn=kms_key.key_arn),
            resources=["secrets"],
        )
    ],
    ...
    tags=[aws_native.eks.ClusterTagArgs(key="Name", value=eks_cluster_name)],
)

# Create EKS Node Group
node_group = aws_native.eks.Nodegroup(
    f"nodeGroup-{stack_name}",
    ...
    tags=[aws_native.eks.NodegroupTagArgs(key="Name", value=nodegroup_name)],
)

# Create VPC Flow Logs
flow_logs_role = aws_native.iam.Role(
    f"vpcFlowLogsRole-{stack_name}",
    assume_role_policy_document=json.dumps(
        {
            "Version": "2012-10-17",
            "Statement": [
...
flow_log_group = aws_native.logs.LogGroup(
    f"vpcFlowLogGroup-{stack_name}",
    log_group_name=f"/aws/vpc/flowlogs/{vpc.id}",
    retention_in_days=90,
    tags=[
        aws_native.logs.LogGroupTagArgs(
            key="Name", value=f"vpcFlowLogGroup-{stack_name}"
        )
    ],
)

vpc_flow_log = aws_native.ec2.FlowLog(
    f"vpcFlowLog-{stack_name}",
    resource_id=vpc.id,
    resource_type="VPC",
    traffic_type="ALL",
...
kubeconfig = pulumi.Output.all(
    eks_cluster.name, eks_cluster.endpoint, eks_cluster.certificate_authority
).apply(
    lambda args: generate_kubeconfig(
        args[0], args[1], base64.b64decode(args[2]["data"]).decode("utf-8")
    )
)

# Create Kubernetes Provider
k8s_provider = k8s.Provider(f"k8sProvider-{stack_name}", kubeconfig=kubeconfig)
...
fluent_bit_namespace = k8s.core.v1.Namespace(
...
fluent_bit_service_account = k8s.core.v1.ServiceAccount(
    "fluent-bit-sa",
    metadata={"name": "fluent-bit", "namespace": "logging"},
    opts=pulumi.ResourceOptions(provider=k8s_provider),
)

fluent_bit_cluster_role = k8s.rbac.v1.ClusterRole(
...
fluent_bit_cluster_role_binding = k8s.rbac.v1.ClusterRoleBinding(
...
fluent_bit_daemonset = k8s.apps.v1.DaemonSet(
