...
def deploy_local_path_storage(
    k8s_provider: k8s.Provider, namespace: str, default_path: str
):
    ...
    def configmap_transformation(obj):
        if (
            obj["kind"] == "ConfigMap"
            and obj["metadata"]["name"] == "local-path-config"
        ):
    ...
    rancher_local_path_provisioner = k8s.yaml.ConfigFile(
        ...
        transformations=[configmap_transformation, storageclass_transformation],
